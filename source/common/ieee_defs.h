...
// Fixed part of the vendor beacon info (Nintendo Co., Ltd.). It is followed by
// "extra_data_size" bytes of game-specific data.
//

//
//     fixed_id        magicNumber (u16, always 0x0001), ver (u8, 1),
//                     platform (u8: 0 = DS, 8 = Wii)
//     game_id         ggid
//     stream_code     tgid
//     extra_data_size userGameInfoLength (up to 112)
//     beacon_type     attribute, a bitfield: 1 = accepting entries,
//                     2 = multiboot, 4 = key sharing, 8 = continuous send
//     cmd_data_size   parentMaxSize
//     reply_data_size childMaxSize
//
// The two size fields really are in bytes, contrary to what this file used to
// say. Nintendo caps both at 512 (510 for multiboot).
// Bits of the "attribute" byte above, which is beacon_type in the struct below.
#define DLPLAY_ATTR_ACCEPTING_ENTRIES   (1 << 0)
#define DLPLAY_ATTR_MULTIBOOT           (1 << 1)
#define DLPLAY_ATTR_KEY_SHARING         (1 << 2)
#define DLPLAY_ATTR_CONTINUOUS_SEND     (1 << 3)
...
typedef struct {
    ...
    u8 lcd_video_sync[2];
    u8 fixed_id[4]; // magicNumber, ver, platform. See above.
    u8 game_id[4];
    u8 stream_code[2];
    u8 extra_data_size;
    u8 beacon_type; // Bitfield, not an enum. See above.
    u8 cmd_data_size[2]; // Size in bytes
    u8 reply_data_size[2]; // Size in bytes
} FieVendorNintendoHeader;

static_assert(sizeof(FieVendorNintendoHeader) == 24);

// Vendor beacon info as generated by DSWifi in local multiplayer mode.
typedef struct {
    FieVendorNintendoHeader hdr;
    DSWifiExtraData extra_data;
} FieVendorNintendo;

static_assert(sizeof(FieVendorNintendo) == 48);

// Offsets inside the Nintendo vendor information element, from the first byte
// of the OUI. They are used to patch the element in MAC RAM while the beacon
// is being transmitted.
#define FIE_NINTENDO_OFS_EXTRA_DATA_SIZE    0x12
#define FIE_NINTENDO_OFS_BEACON_TYPE        0x13
#define FIE_NINTENDO_OFS_EXTRA_DATA         0x18

static_assert(FIE_NINTENDO_OFS_EXTRA_DATA == sizeof(FieVendorNintendoHeader));

// DSWIFI_BEACON_EXTRA_DATA_MAX and DSWIFI_BEACON_MAX_FRAGMENTS are defined in
// dswifi_common.h because they are part of the public API.
