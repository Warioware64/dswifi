...
// Default ARM7 core for this fork of DSWiFi.
//
// The same core BlocksDS ships, built against this library instead of the one
// that comes with it. The two are not interchangeable: this fork changes both
// the MAC RAM layout and Wifi_MainStruct, which the ARM9 and the ARM7 have to
// agree on, so pairing a stock core with an ARM9 built against this library
// links without complaint and then misbehaves. Wifi_Init() checks for that and
// refuses rather than running on.
//
// Nothing here is specific to this fork: the headers keep the names they have
// upstream, so this is the stock source unchanged.
